// Console input and output.
// Input is from the keyboard or serial port.
// Output is written to the screen and serial port.

#include "types.h"
#include "defs.h"
#include "param.h"
#include "traps.h"
#include "spinlock.h"
#include "sleeplock.h"
#include "fs.h"
#include "file.h"
#include "memlayout.h"
#include "mmu.h"
#include "proc.h"
#include "x86.h"

#define VGA_0x03_MEMORY P2V(0xb8000)
#define VGA_0x03_WIDTH 80
#define VGA_0x03_HEIGHT 25

#define VGA_0x13_MEMORY P2V(0xA0000)
#define VGA_0x13_WIDTH 320
#define VGA_0x13_HEIGHT 200

#define VGA_0x12_WIDTH 640
#define VGA_0x12_HEIGHT 480

static void consputc(int);

static int panicked = 0;
uint currentvgamode = 0x03;

/**
 * Global console state shared between all processes and CPUs.
 */
static struct {
    struct spinlock lock;
    ushort textbuffer[VGA_0x03_WIDTH * VGA_0x03_HEIGHT];
    int cursorpos;
    bool returnedtotext;
    int locking;
} cons;

// Copies text memory to a buffer, text glyphs are 16 bits.
void backuptext(){
    memmove(&cons.textbuffer, VGA_0x03_MEMORY, VGA_0x03_WIDTH * VGA_0x03_HEIGHT * 2);
}

void restoretext(){
    // Acquire lock since we are writing to hardware
    int locking = cons.locking;
    if (locking) {
        acquire(&cons.lock);
    }

    // Restores text memory from the buffer
    memmove(VGA_0x03_MEMORY, &cons.textbuffer, VGA_0x03_WIDTH * VGA_0x03_HEIGHT * 2);
    // Sets flag telling cgaputc to use saved hardware cursor pos on next call
    cons.returnedtotext = true;

    // Release lock when we are done with it
    if (locking) {
        release(&cons.lock);
    }
}

// Sets pixel on the screen by writing to video memory
void setpixel(int x, int y, int colour){
    // Acquire lock since we are writing to hardware
    int locking = cons.locking;
    if (locking) {
        acquire(&cons.lock);
    }

    // How we plot pixels differs depending on which graphics mode we are in
    switch (currentvgamode){
        case 0x13:
            // Check if in bounds of screen
            if (x >= 0 && x < VGA_0x13_WIDTH && y >= 0 && y < VGA_0x13_HEIGHT){
                // Memory location of pixel = A00000 + 320 * y + x
                memset(VGA_0x13_MEMORY + VGA_0x13_WIDTH * y + x, colour, 1); 
            } 
            break;
        case 0x12:
            // Check if in bounds of screen
            if (x >= 0 && x < VGA_0x12_WIDTH && y >= 0 && y < VGA_0x12_HEIGHT){
                int shift;
                int offset;
                int mask;
                uchar* currentbyte;
                // Offset is which byte we need, will be added to the memory location of the specified video plane
                offset = (VGA_0x12_WIDTH * y + x) / 8;
                /* Shift is used to line our mask up with the correct bit
                We can only write to one byte at a time minimum so we must use a mask to select the 
                bit we want to modify since each pixel uses only one bit per plane */
                shift = 7 - ((VGA_0x12_WIDTH * y + x) % 8);
                // Loop through each plane (0-3)
                for (int i = 0; i < 4; i++){
                    consolevgaplane(i);
                    // Byte we need is found at the memory location of the plane + offset
                    currentbyte = consolevgabuffer() + offset;
                    /* colour & (0x01 << i) selects the bit needed for this plane from the colour
                    >> i then ensures that this bit is the far right bit
                    << shift then shifts the bit to the required bit for the pixel within the current byte*/
                    mask = (colour & (0x01 << i)) >> i << shift;
                    /* Without the branch, colours will merge and blacks will not work.
                    to fix this we use a conditional statement where if mask != 0, we do currentbyte OR mask
                    if the mask is 0, we do currentbyte AND !(bitmask)*/
                    mask ? (*currentbyte |= mask) : (*currentbyte &= ~(0x01 << shift));
                }
            }
            break;
    }

    // Release lock when we are done with it
    if (locking) {
        release(&cons.lock);
    }
}

// Fills video memory with zeros to clear the screen
void clearscreen(){
    // Acquire lock since we are writing to hardware
    int locking = cons.locking;
    if (locking) {
        acquire(&cons.lock);
    }

    switch(currentvgamode){
        case 0x13:
            memset(VGA_0x13_MEMORY, 0, VGA_0x13_WIDTH * VGA_0x13_HEIGHT);  
            break;
        case 0x12:
            for (int plane = 0; plane < 4; plane++){
                consolevgaplane(plane);
                memset(consolevgabuffer(), 0, VGA_0x12_WIDTH * VGA_0x12_HEIGHT / 8); 
            }
            break;
    }

    // Release lock when we are done with it
    if (locking) {
        release(&cons.lock);
    }
}

// Draws filled rectangle to the screen at set position with set width and height
void fillrect(int x, int y, int width, int height, int colour){

    // Acquire lock since we are writing to hardware
    int locking = cons.locking;
    if (locking) {
        acquire(&cons.lock);
    }

    switch(currentvgamode){
        case 0x13:
            // Check if in bounds of screen
            if (x >= 0 && x + width <= VGA_0x13_WIDTH && y >= 0 && y + height <= VGA_0x13_HEIGHT){
                // Fill memory for each row
                for (int row = y; row < y + height; row++){
                    memset(VGA_0x13_MEMORY + VGA_0x13_WIDTH * row + x, colour, width);  
                }
            }
            break;
        case 0x12:
            // Check if in bounds of screen
            if (x >= 0 && x + width <= VGA_0x12_WIDTH && y >= 0 && y + height <= VGA_0x12_HEIGHT){
                int shift;
                int offset;
                int mask;
                uchar* currentbyte;
                int column;
                int bytestofill;
                int pmask;
                // Loop through each row
                for (int row = y; row < y + height; row++){
                    column = x; 
                    offset = (VGA_0x12_WIDTH * row + column) / 8;
                    shift = 7 - ((VGA_0x12_WIDTH * row + column) % 8);

                    // Set bits that are before the first whole byte
                    // Example:
                    // We want to set bits 5-15
                    // Bits 5, 6, 7 are not a full byte so we set these first here
                    // The remaining byte 8-15 can then be set in one go afterwards
                    // Bits/bytes are set using a modified version of the method used in setpixel
                    if (shift != 7){
                        // Loop through each plane (0-3)
                        for (int i = 0; i < 4; i++){
                            consolevgaplane(i);
                            currentbyte = consolevgabuffer() + offset;
                            mask = (colour & (0x01 << i)) >> i;
                            pmask = 0x01;
                            for (int i = 0; i < shift; i++){
                                // If mask is not 0, fill with shift number of 1's from the right
                                // Do the same with pmask, ignoring whether it is 0 or not
                                if (mask){
                                    mask = (mask << 1) + 1;
                                }
                                pmask = (pmask << 1) + 1;
                            }
                            // Make sure we do not override current memory unless the colour is black
                            mask ? (*currentbyte |= mask) : (*currentbyte &= ~pmask);
                        }
                        // Jump to next full byte
                        column += (shift + 1);
                    }

                    // Calculate how many full bytes we have on this row
                    bytestofill = ((x + width) - column) / 8;
                    offset = (VGA_0x12_WIDTH * row + column) / 8;

                    // Loop through each plane (0-3)
                    for (int i = 0; i < 4; i++){
                        // We are always filling an entire byte here so we do not need to worry about overwriting
                        mask = colour & (0x01 << i);
                        mask = mask ? 255 : 0;
                        consolevgaplane(i);
                        currentbyte = consolevgabuffer() + offset;
                        memset(currentbyte, mask, bytestofill); 
                    }

                    // Jump past bytes we just filled
                    column += bytestofill * 8;

                    // Like earlier, we are now setting the trailing bits that do not make up a full byte
                    // The only difference is that the mask and pmask are shifted to the right now instead of the left as before
                    if (column != x + width){
                        offset = (VGA_0x12_WIDTH * row + column) / 8;
                        shift = (x + width) - column;
                        // Loop through each plane (0-3)
                        for (int i = 0; i < 4; i++){
                            consolevgaplane(i);
                            currentbyte = consolevgabuffer() + offset;
                            mask = colour & (0x01 << i);
                            mask = mask ? 128 : 0;
                            pmask = 128;
                            for (int i = 0; i < shift; i++){
                                if (mask){
                                    mask = (mask >> 1);
                                    mask += 128;
                                }
                                pmask = (pmask >> 1) + 128;
                            }
                            mask ? (*currentbyte |= mask) : (*currentbyte &= ~pmask);
                        }
                    }
                }
            }
            break;
    }

    // Release lock when we are done with it
    if (locking) {
        release(&cons.lock);
    }
}

static void printint(int xx, int base, int sign) {
    static char digits[] = "0123456789abcdef";
    char buf[16];
    int i;
    uint x;

    if (sign && (sign = xx < 0)) {
        x = -xx;
    }
    else {
        x = xx;
    }

    i = 0;
    do {
        buf[i++] = digits[x % base];
    }
    while ((x /= base) != 0);

    if (sign) {
        buf[i++] = '-';
    }

    while (--i >= 0) {
        consputc(buf[i]);
    }
}

// Print to the console. only understands %d, %x, %p, %s.
void cprintf(char *fmt, ...) {
    int i, c, locking;
    uint *argp;
    char *s;

    locking = cons.locking;
    if (locking) {
        acquire(&cons.lock);
    }

    if (fmt == 0) {
        panic("null fmt");
    }

    argp = (uint*)(void*)(&fmt + 1);
    for (i = 0; (c = fmt[i] & 0xff) != 0; i++) {
        if (c != '%') {
            consputc(c);
            continue;
        }
        c = fmt[++i] & 0xff;
        if (c == 0) {
            break;
        }
        switch (c) {
            case 'd':
                printint(*argp++, 10, 1);
                break;
            case 'x':
            case 'p':
                printint(*argp++, 16, 0);
                break;
            case 's':
                if ((s = (char*)*argp++) == 0) {
                    s = "(null)";
                }
                for (; *s; s++) {
                    consputc(*s);
                }
                break;
            case '%':
                consputc('%');
                break;
            default:
                // Print unknown % sequence to draw attention.
                consputc('%');
                consputc(c);
                break;
        }
    }

    if (locking) {
        release(&cons.lock);
    }
}

void panic(char *s) {
    int i;
    uint pcs[10];

    cli();
    cons.locking = 0;
    // use lapiccpunum so that we can call panic from mycpu()
    cprintf("lapicid %d: panic: ", lapicid());
    cprintf(s);
    cprintf("\n");
    getcallerpcs(&s, pcs);
    for (i = 0; i < 10; i++) {
        cprintf(" %p", pcs[i]);
    }
    panicked = 1; // freeze other CPU
    for (;;) {
        ;
    }
}

#define BACKSPACE 0x100
#define CRTPORT 0x3d4

static void cgaputc(int c) {
    int pos;
    ushort* crt = VGA_0x03_MEMORY;

    if(!cons.returnedtotext && currentvgamode == 0x03){
        // Cursor position: col + 80*row.
        outb(CRTPORT, 14);
        pos = inb(CRTPORT + 1) << 8;
        outb(CRTPORT, 15);
        pos |= inb(CRTPORT + 1);
    }
    else{
        // If returning to text or in graphics mode, use saved pos rather than fetch from port.
        pos = cons.cursorpos;
        cons.returnedtotext = false;
    }

    if (c == '\n') {
        pos += 80 - pos % 80;
    }
    else if (c == BACKSPACE) {
        if (pos > 0) {
            --pos;
        }
    }
    else {
        if (currentvgamode == 0x03){
            crt[pos++] = (c & 0xff) | 0x0700;  // black on white
        }
        else {
            // If in video mode, write text to buffer rather than video memory.
            cons.textbuffer[pos++] = (c & 0xff) | 0x0700;
        }
    }
    if (pos < 0 || pos > 25 * 80) {
        panic("pos under/overflow");
    }

    if ((pos / 80) >= 24) { // Scroll up.
        memmove(crt, crt + 80, sizeof(crt[0]) * 23 * 80);
        pos -= 80;
        memset(crt + pos, 0, sizeof(crt[0]) * (24 * 80 - pos));
    }

    outb(CRTPORT, 14);
    outb(CRTPORT + 1, pos >> 8);
    outb(CRTPORT, 15);
    outb(CRTPORT + 1, pos);
    crt[pos] = ' ' | 0x0700;
    // Save hardware cursor pos
    cons.cursorpos = pos;
}

void consputc(int c) {
    if (panicked) {
        cli();
        for (;;) {
            ;
        }
    }

    if (c == BACKSPACE) {
        uartputc('\b');
        uartputc(' ');
        uartputc('\b');
    }
    else {
        uartputc(c);
    }
    cgaputc(c);
}

int consoleget(void) {
    int c;

    acquire(&cons.lock);

    while ((c = kbdgetc()) <= 0) {
        if (c == 0) {
            c = kbdgetc();
        }
    }

    release(&cons.lock);

    return c;
}

#define INPUT_BUF 128
struct {
    char buf[INPUT_BUF];
    uint r;  // Read index
    uint w;  // Write index
    uint e;  // Edit index
} input;

#define C(x)  ((x) - '@')  // Control-x

void consoleintr(int (*getc)(void)) {
    int c, doprocdump = 0;

    acquire(&cons.lock);
    while ((c = getc()) >= 0) {
        switch (c) {
            case C('P'):  // Process listing.
                // procdump() locks cons.lock indirectly; invoke later
                doprocdump = 1;
                break;
            case C('U'):  // Kill line.
                while (input.e != input.w &&
                       input.buf[(input.e - 1) % INPUT_BUF] != '\n') {
                    input.e--;
                    consputc(BACKSPACE);
                }
                break;
            case C('H'):
            case '\x7f':  // Backspace
                if (input.e != input.w) {
                    input.e--;
                    consputc(BACKSPACE);
                }
                break;
            default:
                if (c != 0 && input.e - input.r < INPUT_BUF) {
                    c = (c == '\r') ? '\n' : c;
                    input.buf[input.e++ % INPUT_BUF] = c;
                    consputc(c);
                    if (c == '\n' || c == C('D') || input.e == input.r + INPUT_BUF) {
                        input.w = input.e;
                        wakeup(&input.r);
                    }
                }
                break;
        }
    }
    release(&cons.lock);
    if (doprocdump) {
        procdump();  // now call procdump() wo. cons.lock held
    }
}

int consoleread(struct inode *ip, char *dst, int n) {
    uint target;
    int c;

    iunlock(ip);
    target = n;
    acquire(&cons.lock);
    while (n > 0) {
        while (input.r == input.w) {
            if (myproc()->killed) {
                release(&cons.lock);
                ilock(ip);
                return -1;
            }
            sleep(&input.r, &cons.lock);
        }
        c = input.buf[input.r++ % INPUT_BUF];
        if (c == C('D')) { // EOF
            if (n < target) {
                // Save ^D for next time, to make sure
                // caller gets a 0-byte result.
                input.r--;
            }
            break;
        }
        *dst++ = c;
        --n;
        if (c == '\n') {
            break;
        }
    }
    release(&cons.lock);
    ilock(ip);

    return target - n;
}

int consolewrite(struct inode *ip, char *buf, int n) {
    int i;

    iunlock(ip);
    acquire(&cons.lock);
    for (i = 0; i < n; i++) {
        consputc(buf[i] & 0xff);
    }
    release(&cons.lock);
    ilock(ip);

    return n;
}

void consoleinit(void) {
    initlock(&cons.lock, "console");

    devsw[CONSOLE].write = consolewrite;
    devsw[CONSOLE].read = consoleread;
    cons.locking = 1;

    ioapicenable(IRQ_KBD, 0);
}

// Video mode switching and handling routines.
//
// Based on public domain code written by Chris Giese, modified to work with xv6.

#define	VGA_AC_INDEX		0x3C0
#define	VGA_AC_WRITE		0x3C0
#define	VGA_AC_READ			0x3C1
#define	VGA_MISC_WRITE		0x3C2
#define VGA_SEQ_INDEX		0x3C4
#define VGA_SEQ_DATA		0x3C5
#define	VGA_DAC_READ_INDEX	0x3C7
#define	VGA_DAC_WRITE_INDEX	0x3C8
#define	VGA_DAC_DATA		0x3C9
#define	VGA_MISC_READ		0x3CC
#define VGA_GC_INDEX 		0x3CE
#define VGA_GC_DATA 		0x3CF
/*						COLOR emulation		MONO emulation */
#define VGA_CRTC_INDEX		0x3D4			 /* 0x3B4 */
#define VGA_CRTC_DATA		0x3D5			 /* 0x3B5 */
#define	VGA_INSTAT_READ		0x3DA

#define	VGA_NUM_SEQ_REGS	5
#define	VGA_NUM_CRTC_REGS	25
#define	VGA_NUM_GC_REGS		9
#define	VGA_NUM_AC_REGS		21
#define	VGA_NUM_REGS		(1 + VGA_NUM_SEQ_REGS + VGA_NUM_CRTC_REGS + VGA_NUM_GC_REGS + VGA_NUM_AC_REGS)

uchar registers_80x25_text[] =
{
    /* Miscellaneous registers */
    0x67,
    /* Sequencer registers */
    0x03, 0x00, 0x03, 0x00, 0x02,
    /* CRTC registers */
    0x5F, 0x4F, 0x50, 0x82, 0x55, 0x81, 0xBF, 0x1F,
    0x00, 0x4F, 0x0D, 0x0E, 0x00, 0x00, 0x00, 0x50,
    0x9C, 0x0E, 0x8F, 0x28, 0x1F, 0x96, 0xB9, 0xA3,
    0xFF,
    /* Graphics Controller registers */
    0x00, 0x00, 0x00, 0x00, 0x00, 0x10, 0x0E, 0x00,
    0xFF,
    /* Attribute Controller registers */
    0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x14, 0x07,
    0x38, 0x39, 0x3A, 0x3B, 0x3C, 0x3D, 0x3E, 0x3F,
    0x0C, 0x00, 0x0F, 0x08, 0x00
};

uchar registers_320x200x256[] =
{
    /* Miscellaneous registers */
    0x63,
    /* Sequencer registers */
    0x03, 0x01, 0x0F, 0x00, 0x0E,
    /* CRTC registers */
    0x5F, 0x4F, 0x50, 0x82, 0x54, 0x80, 0xBF, 0x1F,
    0x00, 0x41, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x9C, 0x0E, 0x8F, 0x28,	0x40, 0x96, 0xB9, 0xA3,
    0xFF,
    /* Graphics Controller registers */
    0x00, 0x00, 0x00, 0x00, 0x00, 0x40, 0x05, 0x0F,
    0xFF,
    /* Attribute Controller registers */
    0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07,
    0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F,
    0x41, 0x00, 0x0F, 0x00,	0x00
};

uchar registers_640x480x16[] =
{
    /* Miscellaneous registers */
    0xE3,
    /* Sequencer registers */
    0x03, 0x01, 0x08, 0x00, 0x06,
    /* CRTC registers */
    0x5F, 0x4F, 0x50, 0x82, 0x54, 0x80, 0x0B, 0x3E,
    0x00, 0x40, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0xEA, 0x0C, 0xDF, 0x28, 0x00, 0xE7, 0x04, 0xE3,
    0xFF,
    /* Graphics Controller registers */
    0x00, 0x00, 0x00, 0x00, 0x03, 0x00, 0x05, 0x0F,
    0xFF,
    /* Attribute Controller registers */
    0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x14, 0x07,
    0x38, 0x39, 0x3A, 0x3B, 0x3C, 0x3D, 0x3E, 0x3F,
    0x01, 0x00, 0x0F, 0x00, 0x00
};

uchar font_8x16[4096] =
{
    
};

/**
 * Set video plane.
 *
 * On entry, plane must be set to a value between 0 and 3.
 */
void consolevgaplane(uchar plane) {
    uchar planeMask;

    plane &= 3;
    planeMask = 1 << plane;
    // Set read plane
    outb(VGA_GC_INDEX, 4);
    outb(VGA_GC_DATA, plane);
    // Set write plane
    outb(VGA_SEQ_INDEX, 2);
    outb(VGA_SEQ_DATA, planeMask);
}

// Write the specified values to the VGA registers
static void writeVideoRegisters(uchar* regs) {
    unsigned i;

    /* write MISCELLANEOUS reg */
    outb(VGA_MISC_WRITE, *regs);
    regs++;
    /* write SEQUENCER regs */
    for (i = 0; i < VGA_NUM_SEQ_REGS; i++)
    {
        outb(VGA_SEQ_INDEX, i);
        outb(VGA_SEQ_DATA, *regs);
        regs++;
    }
    /* Unlock CRTC registers */
    outb(VGA_CRTC_INDEX, 0x03);
    outb(VGA_CRTC_DATA, inb(VGA_CRTC_DATA) | 0x80);
    outb(VGA_CRTC_INDEX, 0x11);
    outb(VGA_CRTC_DATA, inb(VGA_CRTC_DATA) & ~0x80);
    /* Make sure they remain unlocked */
    regs[0x03] |= 0x80;
    regs[0x11] &= ~0x80;
    /* write CRTC regs */
    for (i = 0; i < VGA_NUM_CRTC_REGS; i++)
    {
        outb(VGA_CRTC_INDEX, i);
        outb(VGA_CRTC_DATA, *regs);
        regs++;
    }
    /* write GRAPHICS CONTROLLER regs */
    for (i = 0; i < VGA_NUM_GC_REGS; i++)
    {
        outb(VGA_GC_INDEX, i);
        outb(VGA_GC_DATA, *regs);
        regs++;
    }
    /* write ATTRIBUTE CONTROLLER regs */
    for (i = 0; i < VGA_NUM_AC_REGS; i++)
    {
        (void)inb(VGA_INSTAT_READ);
        outb(VGA_AC_INDEX, i);
        outb(VGA_AC_WRITE, *regs);
        regs++;
    }
    /* lock 16-color palette and unblank display */
    (void)inb(VGA_INSTAT_READ);
    outb(VGA_AC_INDEX, 0x20);
}

void writeFont(uchar * fontBuffer, unsigned int fontHeight) {
    uchar		 seq2, seq4, gc4, gc5, gc6;
    unsigned int i;
    uchar*	 fontBase;

    // Save VGA registers
    outb(VGA_SEQ_INDEX, 2);
    seq2 = inb(VGA_SEQ_DATA);

    outb(VGA_SEQ_INDEX, 4);
    seq4 = inb(VGA_SEQ_DATA);
    outb(VGA_SEQ_DATA, seq4 | 0x04);

    outb(VGA_GC_INDEX, 4);
    gc4 = inb(VGA_GC_DATA);

    outb(VGA_GC_INDEX, 5);
    gc5 = inb(VGA_GC_DATA);
    outb(VGA_GC_DATA, gc5 & ~0x10);

    outb(VGA_GC_INDEX, 6);
    gc6 = inb(VGA_GC_DATA);
    outb(VGA_GC_DATA, gc6 & ~0x02);

    consolevgaplane(2);
    // Write font to video memory
    fontBase = (uchar*)P2V(0xB8000);
    for (i = 0; i < 256; i++)
    {
        memmove((ushort*)fontBase, fontBuffer, fontHeight);
        fontBase += 32;
        fontBuffer += fontHeight;
    }
    // Restore VGA registers
    outb(VGA_SEQ_INDEX, 2);
    outb(VGA_SEQ_DATA, seq2);
    outb(VGA_SEQ_INDEX, 4);
    outb(VGA_SEQ_DATA, seq4);
    outb(VGA_GC_INDEX, 4);
    outb(VGA_GC_DATA, gc4);
    outb(VGA_GC_INDEX, 5);
    outb(VGA_GC_DATA, gc5);
    outb(VGA_GC_INDEX, 6);
    outb(VGA_GC_DATA, gc6);
}

/**
 * Video mode switching function which must be called from kernel-space, returning `0` if a valid
 * mode was requested, otherwise `-1`.
 *
 * Currently, only these modes are supported:
 *   0x03: 80x25 text mode.
 *   0x12: 640x480x16 graphics mode.
 *   0x13: 320x200x256 graphics mode.
 */
int consolevgamode(int vgamode) {
    acquire(&cons.lock);

    int errorcode = -1;

    switch (vgamode)
    {
        case 0x03: {
            writeVideoRegisters(registers_80x25_text);
            writeFont(font_8x16, 16);

            currentvgamode = 0x03;
            errorcode = 0;
        } break;

        case 0x12: {
            writeVideoRegisters(registers_640x480x16);

            currentvgamode = 0x12;
            errorcode = 0;
        } break;

        case 0x13: {
            writeVideoRegisters(registers_320x200x256);

            currentvgamode = 0x13;
            errorcode = 0;
        } break;
    }

    release(&cons.lock);

    return errorcode;
}

/**
 * Get the virtual base address of the video memory associated with the current video plane.
 *
 * Note the mapping of CPU memory to video memory is not immediately obvious and, at first sight,
 * this routine looks like it gives an incorrect value for plane 1. However, reading the article at
 * http://www.osdever.net/FreeVGA/vga/vgamem.htm will give you more insight into what is going on.
 *
 * Returns a pointer to the virtual address (NOT the physical address) associated with the current
 * video plane.
 */
uchar* consolevgabuffer() {
    uchar* base;
    uchar plane;

    outb(VGA_GC_INDEX, 6);

    plane = inb(VGA_GC_DATA);
    plane >>= 2;
    plane &= 3;

    switch (plane)
    {
    case 0:
    case 1:
        base = (uchar*)P2V(0xA0000);
        break;

    case 2:
        base = (uchar*)P2V(0xB0000);
        break;

    case 3:
        base = (uchar*)P2V(0xB8000);
        break;
    }

    return base;
}
